...
Each `WKOpenVR-<Feature>-v{version}-Setup.exe` turns on ONLY that one feature on first launch. The installer drops the matching `enable_<feature>.flag` into the driver's resources directory so the feature is active the next time SteamVR starts. The plain `WKOpenVR-v{version}-Setup.exe` enables nothing by default -- it gives you the umbrella with every feature available, none turned on.
