...
inline bool QuatNormalize(const double q[4], double out[4])
{
    if (!(std::isfinite(q[0]) && std::isfinite(q[1]) &&
        ...
        out[0] = 1.0; out[1] = out[2] = out[3] = 0.0;
        return false;
    }
...
inline void QuatNlerpShortest(const double from[4], const double to[4], double alpha, double out[4])
{
    ...
    if (!okF) { out[0] = nt[0]; out[1] = nt[1]; out[2] = nt[2]; out[3] = nt[3]; return; }
    if (!okT) { out[0] = nf[0]; out[1] = nf[1]; out[2] = nf[2]; out[3] = nf[3]; return; }
...
inline StepResult FilterStep(FilterState& s,
                             const Params& p,
                             const double rawPos[3],
                             const double rawRotIn[4],
                             double reportedLinearSpeed,
                             double reportedAngularSpeed,
                             double rawDtSeconds)
{
    StepResult out;
    ...
    const double posTarget = std::max(
        SmoothStep(p.linStillSpeed, p.linMovingSpeed, s.linearSpeedHat),
        SmoothStep(p.posErrStillM, p.posErrMovingM, posInnovation));
    const double rotTarget = std::max(
        SmoothStep(p.angStillSpeed, p.angMovingSpeed, s.angularSpeedHat),
        SmoothStep(p.rotErrStillRad, p.rotErrMovingRad, rotInnovation));

    s.posRelease = UpdateEnvelope(
        s.posRelease, posTarget, dt, p.gateAttackTauSeconds, p.gateReleaseTauSeconds);
    s.rotRelease = UpdateEnvelope(
        s.rotRelease, rotTarget, dt, p.gateAttackTauSeconds, p.gateReleaseTauSeconds);

    const double posErrSpeed = std::min(
        std::max(0.0, posInnovation - p.posErrStillM) / dt, 3.0);
    const double rotErrSpeed = std::min(
        std::max(0.0, rotInnovation - p.rotErrStillRad) / dt, 20.0);
    ...
    s.filteredRot[0] = newRot[0]; s.filteredRot[1] = newRot[1];
    s.filteredRot[2] = newRot[2]; s.filteredRot[3] = newRot[3];

    s.prevRawPos[0] = rawPos[0]; s.prevRawPos[1] = rawPos[1]; s.prevRawPos[2] = rawPos[2];
    s.prevRawRot[0] = rawRot[0]; s.prevRawRot[1] = rawRot[1];
    s.prevRawRot[2] = rawRot[2]; s.prevRawRot[3] = rawRot[3];
