...
ScenarioResult RunScenario_captions(ScenarioContext &ctx) {
	...
	const fs::path host_exe = ctx.driver_resources
		/ "captions" / "host" / "WKOpenVR.CaptionsHost.exe";
	std::error_code ec;
	const bool host_present = fs::exists(host_exe, ec);
	if (!host_present) {
		ctx.log.Warn("CaptionsHost.exe missing at " + host_exe.string()
			+ "; supervisor will report not-running");
	}
	...
	try {
		...
		if (resp.type != protocol::ResponseSuccess) {
			return Fail("captions", duration_now(),
				"expected ResponseSuccess on SetCaptionsConfig, got type="
				+ std::to_string((int)resp.type));
		}
	} catch (const std::exception &ex) {
		return Fail("captions", duration_now(),
			std::string("SetCaptionsConfig failed: ") + ex.what());
	}
	...
	try {
		...
		if (resp.type != protocol::ResponseCaptionsSupervisorStatus) {
			return Fail("captions", duration_now(),
				"expected ResponseCaptionsSupervisorStatus, got type="
				+ std::to_string((int)resp.type));
		}
		ctx.log.Info("supervisor: host_halted="
			+ std::to_string((int)resp.captionsSupervisorStatus.host_halted)
			+ ", last_exit_code="
			+ std::to_string(resp.captionsSupervisorStatus.last_exit_code));
	} catch (const std::exception &ex) {
		return Fail("captions", duration_now(),
			std::string("CaptionsGetSupervisorStatus failed: ") + ex.what());
	}
