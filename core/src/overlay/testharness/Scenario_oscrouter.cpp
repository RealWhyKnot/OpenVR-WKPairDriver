...
bool WriteOscPublishDatagram(HANDLE pipe, const char *source_id,
	const void *osc_bytes, uint32_t osc_len)
{
	...
	DWORD written = 0;
	if (!::WriteFile(pipe, source, sizeof(source), &written, nullptr) || written != sizeof(source))
		return false;
	if (!::WriteFile(pipe, &osc_len, sizeof(osc_len), &written, nullptr) || written != sizeof(osc_len))
		return false;
	if (osc_len > 0) {
		if (!::WriteFile(pipe, osc_bytes, osc_len, &written, nullptr) || written != osc_len)
			return false;
	}
	return true;
}
...
StubOscPacket MakeStubPacket() {
	...
	p.arg_be = ((v & 0xFFu) << 24) | (((v >> 8) & 0xFFu) << 16)
...
ScenarioResult RunScenario_oscrouter(ScenarioContext &ctx) {
	...
	try {
		...
		return Fail("oscrouter", duration_now(),
			std::string("OscRouteSubscribe failed: ") + ex.what());
	}
	...
	for (int attempt = 0; attempt < 20 && pub == INVALID_HANDLE_VALUE; ++attempt) {
		pub = ::CreateFileA(OPENVR_PAIRDRIVER_OSCROUTER_PUB_PIPE_NAME,
	...
	if (pub == INVALID_HANDLE_VALUE) {
		return Fail("oscrouter", duration_now(),
			"could not open OscRouterPub pipe after retries");
	}
	DWORD mode = PIPE_READMODE_MESSAGE;
	::SetNamedPipeHandleState(pub, &mode, nullptr, nullptr);
	int published = 0;
	for (int i = 0; i < 5; ++i) {
		if (WriteOscPublishDatagram(pub, "harness-osc-source-id-padded-12",
				&packet, sizeof(packet))) ++published;
	}
	::CloseHandle(pub);
	if (published < 5) {
		return Fail("oscrouter", duration_now(),
	...
	try {
		...
		return Fail("oscrouter", duration_now(),
			std::string("OscGetStats failed: ") + ex.what());
	}
