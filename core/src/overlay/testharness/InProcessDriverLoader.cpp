...
void InProcessDriverLoader::Load(const std::filesystem::path &driver_dll,
	MockOpenVRRuntime &runtime)
{
	if (hModule_ != nullptr) {
	...
	HMODULE h = ::LoadLibraryExW(driver_dll.wstring().c_str(), nullptr,
		LOAD_WITH_ALTERED_SEARCH_PATH);
	if (!h) {
		const DWORD err = ::GetLastError();
		throw std::runtime_error("LoadLibraryExW failed for " + driver_dll.string() + ": "
	...
	if (!factory) {
		...
		::FreeLibrary(h);
		hModule_ = nullptr;
		throw std::runtime_error("GetProcAddress(HmdDriverFactory) failed: "
			+ DescribeWin32Error(err));
	}
