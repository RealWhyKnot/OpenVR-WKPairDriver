...
ScenarioResult RunScenario_calibration(ScenarioContext &ctx) {
	...
	try {
		...
		return Fail("calibration", duration_now(),
			std::string("SetDeviceTransform failed: ") + ex.what());
	}
	...
	const uint32_t ref_id = ctx.pose_source.AddDevice("CAL-REF-1",
	...
	const auto added_match = ctx.mock.recorder().CountSince(
		[ref_id](const MockCall &c) {
			return c.kind == MockCallKind::TrackedDeviceAdded && c.device_id == ref_id;
		},
		cursor);
	if (added_match == 0) {
		return Fail("calibration", duration_now(),
