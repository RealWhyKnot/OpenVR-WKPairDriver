...
bool TypeMatches(vr::PropertyTypeTag_t tag, const MockProperties::PropValue &v) {
	switch (tag) {
		case vr::k_unFloatPropertyTag:           return std::holds_alternative<float>(v);
		case vr::k_unInt32PropertyTag:           return std::holds_alternative<int32_t>(v);
		case vr::k_unUint64PropertyTag:          return std::holds_alternative<uint64_t>(v);
		case vr::k_unBoolPropertyTag:            return std::holds_alternative<bool>(v);
		case vr::k_unStringPropertyTag:          return std::holds_alternative<std::string>(v);
		case vr::k_unDoublePropertyTag:          return std::holds_alternative<double>(v);
		case vr::k_unHmdMatrix34PropertyTag:     return std::holds_alternative<vr::HmdMatrix34_t>(v);
...
void MockProperties::SeedString(uint32_t device_id, vr::ETrackedDeviceProperty prop,
	std::string value)
{
...
void MockProperties::SeedInt32(uint32_t device_id, vr::ETrackedDeviceProperty prop,
	int32_t value)
{
	std::lock_guard<std::mutex> lock(mu_);
	const uint64_t c = ContainerForDeviceLocked(device_id);
...
void MockProperties::SeedBool(uint32_t device_id, vr::ETrackedDeviceProperty prop,
	bool value)
{
	std::lock_guard<std::mutex> lock(mu_);
	const uint64_t c = ContainerForDeviceLocked(device_id);
...
vr::PropertyContainerHandle_t MockProperties::TrackedDeviceToPropertyContainer(
	vr::TrackedDeviceIndex_t nDevice)
{
	std::lock_guard<std::mutex> lock(mu_);
	return (vr::PropertyContainerHandle_t)ContainerForDeviceLocked((uint32_t)nDevice);
}

vr::ETrackedPropertyError MockProperties::ReadPropertyBatch(
	vr::PropertyContainerHandle_t ulContainerHandle, vr::PropertyRead_t *pBatch,
	uint32_t unBatchEntryCount)
{
	...
	for (uint32_t i = 0; i < unBatchEntryCount; ++i) {
		...
		if (valueIt == it->second.values.end()) { continue; }
...
vr::ETrackedPropertyError MockProperties::WritePropertyBatch(
	vr::PropertyContainerHandle_t ulContainerHandle, vr::PropertyWrite_t *pBatch,
	uint32_t unBatchEntryCount)
{
	if (!pBatch) return vr::TrackedProp_InvalidDevice;
	std::lock_guard<std::mutex> lock(mu_);
...
const char *MockProperties::GetPropErrorNameFromEnum(vr::ETrackedPropertyError error) {
	switch (error) {
		case vr::TrackedProp_Success:           return "Success";
		case vr::TrackedProp_WrongDataType:     return "WrongDataType";
		case vr::TrackedProp_WrongDeviceClass:  return "WrongDeviceClass";
		case vr::TrackedProp_BufferTooSmall:    return "BufferTooSmall";
		case vr::TrackedProp_UnknownProperty:   return "UnknownProperty";
		case vr::TrackedProp_InvalidDevice:     return "InvalidDevice";
		case vr::TrackedProp_CouldNotContactServer: return "CouldNotContactServer";
		case vr::TrackedProp_ValueNotProvidedByDevice: return "ValueNotProvidedByDevice";
		case vr::TrackedProp_StringExceedsMaximumLength: return "StringExceedsMaximumLength";
		case vr::TrackedProp_NotYetAvailable:   return "NotYetAvailable";
		case vr::TrackedProp_PermissionDenied:  return "PermissionDenied";
		case vr::TrackedProp_InvalidOperation:  return "InvalidOperation";
