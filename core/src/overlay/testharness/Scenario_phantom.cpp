...
ScenarioResult RunScenario_phantom(ScenarioContext &ctx) {
	...
	try {
		...
		if (resp.type != protocol::ResponseSuccess) {
			return Fail("phantom", duration_now(),
				"expected ResponseSuccess on SetPhantomConfig, got type="
				+ std::to_string((int)resp.type));
		}
	} catch (const std::exception &ex) {
		return Fail("phantom", duration_now(),
			std::string("SetPhantomConfig failed: ") + ex.what());
	}
	...
	try {
		...
		return Fail("phantom", duration_now(),
			std::string("SetPhantomDeviceOptIn failed: ") + ex.what());
	}
	...
	try {
		...
		if (resp.type != protocol::ResponseSuccess) {
			return Fail("phantom", duration_now(),
				"expected ResponseSuccess on SetPhantomDeviceRole, got type="
				+ std::to_string((int)resp.type));
		}
	} catch (const std::exception &ex) {
		return Fail("phantom", duration_now(),
			std::string("SetPhantomDeviceRole failed: ") + ex.what());
	}
	...
	for (int attempt = 0; attempt < 30; ++attempt) {
		if (ProbePhantomShmem(magic, version, device_count)) { found = true; break; }
	...
	const uint32_t devId = ctx.pose_source.AddDevice(serial,
