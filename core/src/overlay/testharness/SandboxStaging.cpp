...
void CopyDirRecursive(const fs::path &src, const fs::path &dst) {
	...
	fs::copy(src, dst,
		...
		ec);
	if (ec) {
		throw std::runtime_error("copy_directory failed " + src.string() + " -> " + dst.string()
			+ ": " + ec.message());
	}
}
...
SandboxStaging::~SandboxStaging() {
	...
	if (ec) {
		...
		std::fprintf(stderr, "[testharness] sandbox cleanup left %s behind: %s\n",
			layout_.root.string().c_str(), ec.message().c_str());
	}
}
...
SandboxLayout SandboxStaging::Stage(const StageOptions &options) {
	...
	if (buildDriver.empty()) {
		throw std::runtime_error(
			"failed to locate build/driver_wkopenvr/ relative to the umbrella exe; "
	...
	if (ec) {
		throw std::runtime_error("create_directories on " + layout_.driver_root.string()
			+ " failed: " + ec.message());
	}
	...
	if (options.include_facetracking_host) {
		warnIfMissing(L"facetracking host",
			layout_.driver_resources / L"facetracking" / L"host");
	}
	if (options.include_captions_host) {
		warnIfMissing(L"captions host",
			layout_.driver_resources / L"captions" / L"host");
	}
	if (options.include_phantom_sidecar) {
		warnIfMissing(L"phantom sidecar",
			layout_.driver_resources / L"phantom" / L"host");
	}
