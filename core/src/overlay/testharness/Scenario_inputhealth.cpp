...
ScenarioResult RunScenario_inputhealth(ScenarioContext &ctx) {
	...
	try {
		...
		return Fail("inputhealth", duration_now(),
			std::string("SetInputHealthConfig failed: ") + ex.what());
	}
	...
	for (int attempt = 0; attempt < 20; ++attempt) {
		if (ProbeInputHealthShmem(magic, version, slot_count)) { found = true; break; }
		std::this_thread::sleep_for(std::chrono::milliseconds(50));
	}
	if (!found) {
		return Fail("inputhealth", duration_now(),
			"WKOpenVRInputHealthMemoryV1 shmem segment not visible");
	}
	...
	vr::PropertyContainerHandle_t container = (vr::PropertyContainerHandle_t)
		ctx.mock.properties().TrackedDeviceToPropertyContainer(0);
	vr::VRInputComponentHandle_t handle = 0;
	if (input.CreateScalarComponent(container,
			"/user/hand/left/input/trigger/value", &handle,
			vr::VRScalarType_Absolute, vr::VRScalarUnits_NormalizedOneSided)
		!= vr::VRInputError_None)
	{
		return Fail("inputhealth", duration_now(),
			"CreateScalarComponent failed on mock IVRDriverInput");
	}
	uint64_t cursor = 0;
	for (int i = 0; i < 25; ++i) {
	...
	const auto observed = ctx.mock.recorder().CountSince(
		[](const MockCall &c) {
			return c.kind == MockCallKind::UpdateScalarComponent;
		},
		cursor);
	if (observed < 25) {
		return Fail("inputhealth", duration_now(),
