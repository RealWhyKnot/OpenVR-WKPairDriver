...
uint32_t MockPoseSource::AddDevice(const std::string &serial,
	vr::ETrackedDeviceClass device_class)
{
	std::lock_guard<std::mutex> lock(mu_);
	const bool ok = runtime_.server_driver_host().TrackedDeviceAdded(
		serial.c_str(), device_class, /*pDriver*/ nullptr);
...
void MockPoseSource::PushPose(uint32_t device_id, vr::DriverPose_t pose) {
	...
	// calling PushPose.
	runtime_.server_driver_host().TrackedDevicePoseUpdated(
		device_id, pose, (uint32_t)sizeof(vr::DriverPose_t));
}
