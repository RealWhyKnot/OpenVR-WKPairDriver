...
{
    switch (phase) {
    case SourceBlendPhase::Uninitialized:        return "uninitialized";
    case SourceBlendPhase::SynthStable:          return "synth_stable";
    case SourceBlendPhase::GraceHold:            return "grace_hold";
    case SourceBlendPhase::BlendingToFallback:   return "blend_to_fallback";
    case SourceBlendPhase::FallbackStable:       return "fallback_stable";
    case SourceBlendPhase::WaitingForStableSynth:return "waiting_for_stable_synth";
    case SourceBlendPhase::BlendingToSynth:      return "blend_to_synth";
    }
    return "unknown";
}
...
inline double DurationAlpha(std::chrono::steady_clock::time_point start,
                            std::chrono::steady_clock::time_point now,
                            int64_t durationMs)
{
...
inline double QuatDot(const vr::HmdQuaternion_t& a,
...
inline void SlerpQuat(const vr::HmdQuaternion_t& a,
                      const vr::HmdQuaternion_t& b,
                      double alpha,
                      vr::HmdQuaternion_t& out)
{
    ...
    if (dot < 0.0) {
        bAdj.w = -b.w; bAdj.x = -b.x; bAdj.y = -b.y; bAdj.z = -b.z;
        dot = -dot;
    }
...
inline void BlendPose(const vr::DriverPose_t& a,
                      ...
                      vr::DriverPose_t& out)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        out.vecWorldFromDriverTranslation[i] =
            a.vecWorldFromDriverTranslation[i] * (1.0 - s)
            + b.vecWorldFromDriverTranslation[i] * s;
        out.vecDriverFromHeadTranslation[i] =
            a.vecDriverFromHeadTranslation[i] * (1.0 - s)
            + b.vecDriverFromHeadTranslation[i] * s;
    }
    SlerpQuat(a.qRotation, b.qRotation, s, out.qRotation);
    SlerpQuat(a.qWorldFromDriverRotation, b.qWorldFromDriverRotation,
              s, out.qWorldFromDriverRotation);
    SlerpQuat(a.qDriverFromHeadRotation, b.qDriverFromHeadRotation,
              s, out.qDriverFromHeadRotation);
...
inline void SetPhase(SourceBlendState& state,
                     SourceBlendPhase phase,
                     std::chrono::steady_clock::time_point now)
{
...
inline SourceBlendResult StepSourceBlend(
    SourceBlendState& state,
    const vr::DriverPose_t& fallbackPose,
    const vr::DriverPose_t* synthPose,
    bool synthAvailable,
    std::chrono::steady_clock::time_point now,
    vr::DriverPose_t& out,
    const SourceBlendConfig& config = {})
{
    ...
    if (synthAvailable && synthPose) {
        ...
        if (state.phase == SourceBlendPhase::SynthStable ||
            state.phase == SourceBlendPhase::GraceHold) {
            out = *synthPose;
            state.hasStableSynthSince = true;
            state.stableSynthSince = now;
            SetPhase(state, SourceBlendPhase::SynthStable, now);
    ...
    if (state.phase == SourceBlendPhase::SynthStable) {
        state.blendStartPose =
            state.hasLastGoodSynthPose ? state.lastGoodSynthPose : state.currentPose;
