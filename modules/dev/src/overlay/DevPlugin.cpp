...
DevPlugin::DevPlugin(std::vector<FeaturePlugin *> plugins)
...
void DevPlugin::DrawTab(openvr_pair::overlay::ShellContext &context)
{
	...
	ui::DrawInfoBanner(
		"Dev build",
		"Development-only modules and tools are collected here. Release builds do not compile this tab.");

	ImGui::Spacing();
	ui::DrawSectionHeading("Development modules");
	ui::DrawTextWrapped(
		"These modules are compiled in dev builds and excluded from release builds while they are still in development.");
	ImGui::Spacing();
	...
	openvr_pair::overlay::DrawModuleToggleTable(
		context,
		developmentModules,
		"dev_modules",
