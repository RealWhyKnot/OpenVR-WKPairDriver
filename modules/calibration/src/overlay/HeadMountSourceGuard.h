...
inline HeadMountSampleSource SelectHeadMountSampleSource(
	HeadMountMode mode,
	bool offsetCalibrated)
{
	return (mode >= HeadMountMode::AutoPaired && offsetCalibrated)
		? HeadMountSampleSource::HeadProxy
		: HeadMountSampleSource::PhysicalTracker;
}

inline HeadMountSourceDecision EvaluateHeadMountSourceTransition(
	bool previousValid,
...
{
	if (!previousValid) {
		if (current.source == HeadMountSampleSource::HeadProxy
			&& relativePosCalibrated) {
