...
{
	switch (mode) {
	case HeadMountMode::Off: return "off";
	case HeadMountMode::AutoPaired: return "stabilize";
	case HeadMountMode::Corroborate: return "block_jumps";
	case HeadMountMode::DriverSynth: return "driver_synth";
...
inline OffsetCalibrationPreflight EvaluateOffsetCalibrationPreflight(
	const CalibrationContext& ctx)
{
	...
	if (hm.deviceID < 0) {
		return {
			...
			"Waiting for the headset tracker to appear in SteamVR."
		};
	}
	...
	if (!ctx.validProfile) {
		return {
			false,
			"profile_not_ready",
			"Waiting for a valid continuous calibration profile."
		};
	}
	if (!ctx.relativePosCalibrated) {
		return {
			false,
			"relative_pose_not_ready",
			"Waiting for continuous calibration to lock the relative pose."
		};
	}
	if (HeadMountModeUsesOffsetInContinuous(hm.mode)) {
		return {
			false,
			"head_mount_mode_active",
			"Turn the head-mounted tracker mode Off before solving this offset."
		};
	}
	...
	return {
		true,
		"ready",
		"Ready to solve the headset tracker offset."
	};
}
