...
{
	return spacecal::headmount::SelectHeadMountSampleSource(
		ctx.headMount.mode,
...
static double RotationDeltaRad(const Eigen::Affine3d& previous,
	...
	double& ageMs)
{
	...
	ageMs =
		(static_cast<double>(now.QuadPart - sampleTime.QuadPart) * 1000.0)
...
{
	return spacecal::calibration_speed::SelectObservedFitRmsMm(
		Metrics::error_rawComputed.last(),
		Metrics::error_currentCal.last());
}

static void ClearHeadMountContinuousSourceState(
	CalibrationContext& ctx,
	const char* reason,
	double time,
	HeadMountSampleSource newSource,
	bool previousFingerprintValid,
	HeadMountSampleSource previousSource)
{
	...
	std::snprintf(buf, sizeof buf,
		...
		HeadMountSampleSourceName(previousSource),
		HeadMountSampleSourceName(newSource),
		(int)previousFingerprintValid,
		(int)ctx.headMount.mode,
		ctx.targetID,
		ctx.headMount.deviceID,
		(unsigned)ctx.headMountOffsetVersion,
		(unsigned)ctx.headMountLastSourceOffsetVersion,
		(int)oldRelPosCal,
...
static void UpdateHeadMountSourceFingerprint(
	CalibrationContext& ctx,
	HeadMountSampleSource source)
{
...
static void TickHeadMountSourceTransitionGuard(CalibrationContext& ctx, double time)
{
	...
	const auto decision =
		spacecal::headmount::EvaluateHeadMountSourceTransition(
			previousValid, previous, current, ctx.relativePosCalibrated);
	if (decision.reset) {
		ClearHeadMountContinuousSourceState(
			ctx, decision.reason, time, source, previousValid, previousSource);
	}
	UpdateHeadMountSourceFingerprint(ctx, source);
}

static void LogShadowOffsetBlockedThrottled(
	CalibrationContext& ctx,
	const char* reason,
	double time,
	double profileFitRmsMm,
	double hmdAgeMs,
	double trackerAgeMs)
{
	...
	if (!reasonChanged
		&& (time - g_headMountShadowOffset.lastBlockedLogTime) < 2.0) {
		return;
	}
	g_headMountShadowOffset.lastBlockedReason = reason;
	...
	std::snprintf(buf, sizeof buf,
		...
		reason,
		HeadMountSampleSourceName(CurrentHeadMountSampleSource(ctx)),
		(unsigned)ctx.headMountOffsetVersion,
		(int)ctx.relativePosCalibrated,
		...
		(int)ctx.headMount.autoCorrectOffset,
		(int)wkopenvr::headmount::HeadMountMatchesContinuousTarget(ctx),
		ctx.headMount.deviceID);
	Metrics::WriteLogAnnotation(buf);
}

static void ApplyHeadMountShadowOffset(
	CalibrationContext& ctx,
	const Eigen::AffineCompact3d& candidate,
	const spacecal::headmount::OffsetDelta& savedDelta,
	double residualMm,
	double time)
{
	...
	std::snprintf(buf, sizeof buf,
		...
		(unsigned)ctx.headMountOffsetVersion,
		residualMm,
		savedDelta.translationM * 100.0,
		savedDelta.rotationDeg,
		ctx.calibratedTranslation.norm(),
		(int)(ctx.state == CalibrationState::Continuous),
		(int)ctx.headMount.autoCorrectOffset,
		(unsigned long long)ctx.driverSynthFallbackTotal);
	Metrics::WriteLogAnnotation(buf);

	ClearHeadMountContinuousSourceState(
		ctx, "shadow_offset_applied", time,
		CurrentHeadMountSampleSource(ctx),
		ctx.headMountSourceFingerprintValid,
		ctx.headMountLastSampleSource);
...
static void TickHeadMountShadowOffsetEstimator(CalibrationContext& ctx, double time)
{
	...
	const bool profileHealthy =
		ctx.validProfile
		&& std::isfinite(profileFitRmsMm)
		&& profileFitRmsMm <= kShadowProfileHealthyRmsMm;
	...
	const bool hmdFresh = PoseFreshEnough(
		ctx, vr::k_unTrackedDeviceIndex_Hmd, nowQpc, qpcFrequency, hmdAgeMs);
	const bool trackerFresh = PoseFreshEnough(
		ctx, ctx.headMount.deviceID, nowQpc, qpcFrequency, trackerAgeMs);
	...
	const bool trackerPoseOk =
		trackerInRange
		&& spacecal::headmount::PoseIsRunningOk(
			ctx.devicePoses[ctx.headMount.deviceID]);
	const bool hmdPoseOk =
		spacecal::headmount::PoseIsRunningOk(
			ctx.devicePoses[vr::k_unTrackedDeviceIndex_Hmd]);

	if (!targetMatches || !profileHealthy || !hmdFresh || !trackerFresh
		|| !trackerPoseOk || !hmdPoseOk)
	{
		const char* reason =
			!targetMatches ? "target_mismatch" :
			!profileHealthy ? "profile_unhealthy" :
			(!hmdFresh || !trackerFresh) ? "stale_pose" :
			!hmdPoseOk ? "hmd_pose_invalid" : "tracker_pose_invalid";
		LogShadowOffsetBlockedThrottled(
			ctx, reason, time, profileFitRmsMm, hmdAgeMs, trackerAgeMs);
	...
	if (!g_headMountShadowOffset.collecting) {
		...
		g_headMountShadowOffset.targetFromReferenceAtStart =
			CalibrationTransformFromContext(ctx).inverse();
		g_headMountShadowOffset.windowStartFallbackTotal =
			ctx.driverSynthFallbackTotal;
		g_headMountShadowOffset.windowStartTime = time;
		g_headMountShadowOffset.hasPreviousMotionPose = false;
	}

	const Eigen::Affine3d hmdReference =
		DriverPoseToAffine(ctx.devicePoses[vr::k_unTrackedDeviceIndex_Hmd]);
	const Eigen::Affine3d trackerTarget =
		DriverPoseToAffine(ctx.devicePoses[ctx.headMount.deviceID]);
	const Eigen::Affine3d hmdTarget =
		g_headMountShadowOffset.targetFromReferenceAtStart * hmdReference;

	double derivedLinearSpeedMps = 0.0;
	double derivedAngularSpeedRadps = 0.0;
	if (g_headMountShadowOffset.hasPreviousMotionPose
		&& time > g_headMountShadowOffset.previousMotionTime)
	{
		const double dt = time - g_headMountShadowOffset.previousMotionTime;
		derivedLinearSpeedMps =
			(hmdTarget.translation()
				- g_headMountShadowOffset.previousMotionPose.translation()).norm()
			/ dt;
		derivedAngularSpeedRadps =
			RotationDeltaRad(g_headMountShadowOffset.previousMotionPose, hmdTarget)
			/ dt;
	}
	...
	const double effectiveSpeed = std::max(
		std::isfinite(reportedSpeed) ? reportedSpeed : 0.0,
		std::max(
			derivedLinearSpeedMps,
	...
	if (!readiness.ready
		&& readiness.samplesUsed < kShadowMaxWindowSamplesWithoutReadiness) {
		return;
	}
	...
	if (!readiness.ready) {
		...
		std::snprintf(sbuf, sizeof sbuf,
			...
			HeadMountSampleSourceName(source),
			(unsigned)ctx.headMountOffsetVersion,
			readiness.samplesUsed,
			readiness.residualMm,
			readiness.axisRangeDeg[0],
			readiness.axisRangeDeg[1],
			readiness.axisRangeDeg[2],
			ctx.calibratedTranslation.norm(),
			(int)ctx.headMount.autoCorrectOffset,
			(unsigned long long)(
				ctx.driverSynthFallbackTotal
		...
		LogShadowOffsetBlockedThrottled(
			ctx, "insufficient_motion", time,
			profileFitRmsMm, hmdAgeMs, trackerAgeMs);
		ResetHeadMountShadowOffsetRuntime(/*clearStableWindows=*/false);
		return;
	}
	...
	const OffsetDelta savedDelta =
		ComputeOffsetDelta(ctx.headMount.headFromTracker, result.headFromTracker);
	const bool hasPrevious = g_headMountShadowOffset.hasLastCandidate;
	const OffsetDelta previousDelta = hasPrevious
		? ComputeOffsetDelta(
			g_headMountShadowOffset.lastCandidate,
			result.headFromTracker)
		: OffsetDelta{};
	...
	const bool sourceSettled =
		(time - ctx.headMountLastSourceResetTime) >= kShadowSourceResetQuietSec;
	const uint64_t fallbackDelta =
		ctx.driverSynthFallbackTotal
		- g_headMountShadowOffset.windowStartFallbackTotal;
	...
	if (residualOk && profileHealthy && fallbackQuiet && sourceSettled
		&& mismatchPlausible && mismatchMeaningful && candidateStable)
	{
	...
	std::snprintf(wbuf, sizeof wbuf,
		...
		hasPrevious ? previousDelta.rotationDeg : -1.0,
		readiness.axisRangeDeg[0],
		readiness.axisRangeDeg[1],
		readiness.axisRangeDeg[2],
		ctx.calibratedTranslation.norm(),
		profileFitRmsMm,
		(unsigned long long)fallbackDelta,
		(int)ctx.headMount.autoCorrectOffset,
		g_headMountShadowOffset.stableWindowCount,
		gate.reason,
		(int)gate.readyToApply,
		(int)gate.wouldApply);
	...
	ApplyHeadMountShadowOffset(ctx, result.headFromTracker, savedDelta,
		result.residualMm, time);
	ResetHeadMountShadowOffsetRuntime(/*clearStableWindows=*/true);
}
...
void CalibrationTick(double time)
{
	...
	if (CalCtx.state == CalibrationState::Continuous) {
		...
		const bool blockStaleRelPose =
			CalCtx.headMountNeedsFreshRelativePose
			&& CalCtx.lockRelativePosition
			&& !CalCtx.relativePosCalibrated;
		calibration.lockRelativePosition =
			CalCtx.lockRelativePosition && !blockStaleRelPose;
		if (blockStaleRelPose) {
			...
			if ((time - s_lastHeadMountRelPoseGuardLog) >= 1.0) {
				...
				std::snprintf(gbuf, sizeof gbuf,
					...
					HeadMountSampleSourceName(
						CurrentHeadMountSampleSource(CalCtx)),
					(unsigned)CalCtx.headMountOffsetVersion,
					(int)CalCtx.lockRelativePosition,
					(int)CalCtx.relativePosCalibrated,
					(int)CalCtx.headMountNeedsFreshRelativePose,
	...
	if (hasPublishableCandidate) {
		...
		if (inContinuousState && ctx.relativePosCalibrated
			&& ctx.headMountNeedsFreshRelativePose) {
