...
Eigen::Affine3d MakePose(double yaw, double pitch, double roll,
...
MakePairs(const Eigen::AffineCompact3d& T, int n, uint32_t seed = 42)
{
...
TEST(Solver, FinishSucceedsOnKnownTransform) {
    ...
    double angleErr = std::acos(
        std::max(-1.0, std::min(1.0, (dR.trace() - 1.0) * 0.5))) * 180.0 / EIGEN_PI;
