...
SynthState MakeState(int mode = 3,
                     int32_t deviceId = 2,
...
TEST(DriverSynth, Tracker_invalid)
{
    ...
    const TrackerSnapshot trackerSnap =
        FreshSnap(MakePose(0.0, 1.7, 0.0, false), now);
...
TEST(DriverSynth, Tracker_stale)
{
    const auto now = clk::now();
    const SynthState state = MakeState();
    const TrackerSnapshot staleTracker =
        StaleSnap(MakePose(0.0, 1.7, 0.0), now);
...
TEST(DriverSynth, Upstream_hmd_staleness_does_not_block_tracker_synth)
{
    const auto now = clk::now();
    const SynthState state = MakeState();
    const TrackerSnapshot freshTracker =
        FreshSnap(MakePose(0.0, 1.7, 0.0), now);
...
TEST(DriverSynth, Tracker_position_is_not_quest_sanity_gated)
{
    ...
    const SynthState state = MakeState();
    const TrackerSnapshot trackerSnap =
        FreshSnap(MakePose(2.0, 1.7, 0.0), now);
...
TEST(DriverSynth, Tracker_snap_for_different_deviceId_rejected)
{
    ...
    const TrackerSnapshot trackerSnap =
        FreshSnap(MakePose(0.0, 1.7, 0.0), now, 7);
