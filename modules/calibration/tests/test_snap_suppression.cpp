...
TEST(SnapSuppression, Speed_gate_max_of_hmd_and_tracker)
{
    ...
    const double result = ss::EffectiveSpeedMps(
        HeadMountMode::Corroborate, hmdSpeed, trackerSpeed);
...
TEST(SnapSuppression, Speed_gate_falls_back_when_tracker_invalid)
{
    ...
    const double result = ss::EffectiveSpeedMps(
        HeadMountMode::Corroborate, hmdSpeed, trackerInvalid);
    EXPECT_DOUBLE_EQ(result, hmdSpeed);
}
...
TEST(SnapSuppression, Speed_gate_off_mode_ignores_tracker)
{
    ...
    EXPECT_DOUBLE_EQ(
        ss::EffectiveSpeedMps(HeadMountMode::Off, hmdSpeed, trackerSpeed),
        hmdSpeed);
}
...
TEST(SnapSuppression, Speed_gate_autopaired_mode_ignores_tracker)
{
    ...
    const double trackerSpeed = 0.95;
    EXPECT_DOUBLE_EQ(
        ss::EffectiveSpeedMps(HeadMountMode::AutoPaired, hmdSpeed, trackerSpeed),
        hmdSpeed);
}
...
TEST(SnapSuppression, Speed_gate_driversynth_uses_tracker)
{
    ...
    EXPECT_DOUBLE_EQ(
        ss::EffectiveSpeedMps(HeadMountMode::DriverSynth, hmdSpeed, trackerSpeed),
        trackerSpeed);
}
...
TEST(SnapSuppression, Jump_detector_classifies_slam_snap)
{
    ...
    EXPECT_TRUE(ss::IsJumpClassifiedAsSnap(
...
TEST(SnapSuppression, Jump_detector_real_jump_passes_through)
{
    ...
    EXPECT_FALSE(ss::IsJumpClassifiedAsSnap(
        HeadMountMode::Corroborate,
        0.50,   // hmd: 50 cm
...
TEST(SnapSuppression, Geometry_shift_coherence_uses_tracker_displacement)
{
    ...
    EXPECT_TRUE(ss::ShouldUseTrackerDisplacement(
        HeadMountMode::Corroborate, 0.015));
}

TEST(SnapSuppression, Geometry_shift_off_mode_uses_hmd_imu)
{
    // Off mode: always use velocity-integrated HMD estimate.
    EXPECT_FALSE(ss::ShouldUseTrackerDisplacement(
        HeadMountMode::Off, 0.015));
}
...
static_assert(ss::kSnapHmdJumpM == 0.30,
    "kSnapHmdJumpM changed from 0.30 -- update the plan spec before tuning");
