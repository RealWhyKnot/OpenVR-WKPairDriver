...
TEST(CalibrationAutoSpeedTest, NonFiniteFitFallsBackToFast) {
    EXPECT_EQ(
        cas::BucketForObservedFitRmsMm(std::numeric_limits<double>::infinity()),
        cas::AutoSpeedBucket::Fast);
    EXPECT_EQ(
        cas::BucketForObservedFitRmsMm(std::numeric_limits<double>::quiet_NaN()),
        cas::AutoSpeedBucket::Fast);
}
...
TEST(CalibrationAutoSpeedTest, CandidateFitRmsBeatsRawMotionSpread) {
    ...
    EXPECT_EQ(
        cas::BucketForObservedFitRmsMm(candidateFitRmsMm),
        cas::AutoSpeedBucket::Fast);
    EXPECT_EQ(
        cas::BucketForObservedFitRmsMm(rawWorldPositionSpreadMm),
        cas::AutoSpeedBucket::VerySlow);
}

TEST(CalibrationAutoSpeedTest, CurrentFitUsedWhenCandidateIsUnavailable) {
    const double selected = cas::SelectObservedFitRmsMm(
