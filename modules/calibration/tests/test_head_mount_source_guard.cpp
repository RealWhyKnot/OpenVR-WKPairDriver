...
TEST(HeadMountSourceGuard, SelectsHeadProxyOnlyWhenOffsetIsCalibrated)
{
    EXPECT_EQ(
        ...
        hm::SelectHeadMountSampleSource(HeadMountMode::Off, false));
    EXPECT_EQ(
        HeadMountSampleSource::PhysicalTracker,
        hm::SelectHeadMountSampleSource(HeadMountMode::AutoPaired, false));
    EXPECT_EQ(
        HeadMountSampleSource::HeadProxy,
        hm::SelectHeadMountSampleSource(HeadMountMode::AutoPaired, true));
    EXPECT_EQ(
        HeadMountSampleSource::HeadProxy,
        hm::SelectHeadMountSampleSource(HeadMountMode::DriverSynth, true));
}
...
TEST(HeadMountSourceGuard, InitialHeadProxyWithRelativePoseForcesReset)
{
    ...
    const auto decision = hm::EvaluateHeadMountSourceTransition(
        false, previous, current, true);
