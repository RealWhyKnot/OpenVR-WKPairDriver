...
TEST(ControllerInputTest, UsesAxisTypeMappingForTrigger) {
	...
	axisTypes[static_cast<int>(vr::k_eControllerAxis_Trigger)] =
		static_cast<int32_t>(vr::k_eControllerAxis_Joystick);
...
TEST(ControllerInputTest, FillsControllersPastFirstEightDevices) {
	...
	int32_t out[2] = { -1, -1 };
	const size_t count = wkopenvr::controller_input::FillControllerIdsForTrackingSystem(
		devices, "lighthouse", out, 2);
