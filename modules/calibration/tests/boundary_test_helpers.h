...
inline Eigen::Vector3d TransformStandingPointToRaw(
    const vr::HmdMatrix34_t& standingToRaw,
    double x,
    double y,
    double z)
{
...
inline void ExpectVertexNear(
    const BoundaryVertex& actual,
    double x,
    double y,
    double z,
...
inline void ExpectMatrixTranslationNear(
    const vr::HmdMatrix34_t& actual,
    float x,
    float y,
    float z,
...
inline void ExpectMatrixRotationUnchanged(
    const vr::HmdMatrix34_t& actual,
    const vr::HmdMatrix34_t& expected,
    float tolerance = kFloatTolerance)
{
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
            EXPECT_NEAR(actual.m[r][c], expected.m[r][c], tolerance)
                << "matrix cell [" << r << "][" << c << "]";
        }
    }
}

inline std::vector<BoundaryVertex> SquareBoundary(
    double minX,
    double minZ,
    double maxX,
    double maxZ,
...
inline void ExpectBoundsNear(
    const wkopenvr::boundary::PolygonBounds& actual,
    double xMin,
    double xMax,
    double zMin,
    double zMax,
