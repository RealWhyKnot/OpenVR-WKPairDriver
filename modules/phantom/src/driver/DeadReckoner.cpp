    ...
    vr::DriverPose_t& out_pose) const
{
    ...
    if (!IsFinite3(newest->pose.vecPosition) ||
        !IsFinite3(newest->pose.vecVelocity) ||
        !IsFinite3(newest->pose.vecAcceleration) ||
        !IsFinite3(newest->pose.vecAngularVelocity) ||
    ...
    const double dt_s = (dt_ns > 0)
        ? static_cast<double>(dt_ns) / static_cast<double>(qpc_freq) * 1.0
        : 0.0;
    ...
    const int64_t age_ms = (qpc_freq > 0)
        ? (dt_ns * 1000) / qpc_freq
        : 0;
    double damp = 1.0 - static_cast<double>(std::max<int64_t>(0, age_ms))
        / static_cast<double>(kFullDampMs);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        out_pose.vecPosition[i] = newest->pose.vecPosition[i]
            + v * dt_s
            + 0.5 * a * dt_s * dt_s;
    ...
    vr::HmdQuaternion_t dq = QFromAxisAngle(
        newest->pose.vecAngularVelocity[0] * dt_s * damp,
