...
DropoutState::DropoutState()
    : timings_(LadderTimings::Defaults())
{
}
...
void DropoutState::OnRealPoseObserved(int64_t qpc_ns,
                                      const PoseHistory& /*history*/,
                                      const vr::DriverPose_t& observed)
{
    ...
    if (observed.result == vr::TrackingResult_Running_OK
        && observed.poseIsValid
        && observed.deviceIsConnected) {
...
void DropoutState::Tick(int64_t qpc_ns, int64_t qpc_freq)
{
    ...
    const int64_t silence_ms = (last_real_qpc_ns_ < 0)
        ? 0
        : QpcMs(qpc_ns - last_real_qpc_ns_, qpc_freq);

    if (state_ == TrackerState::REAL
        && last_real_qpc_ns_ >= 0
        && silence_ms >= timings_.dropout_silence_ms) {
    ...
    if (dropout_start_qpc_ns_ >= 0 && state_ != TrackerState::REAL
        ...
        if (age_ms >= timings_.lost_hold_ms
            && state_ != TrackerState::LOST) {
            state_ = TrackerState::LOST;
    ...
    if (state_ == TrackerState::BLEND_IN && blend_in_start_qpc_ns_ >= 0) {
        ...
        if (blend_age_ms >= timings_.blend_in_ms) {
            if (dropout_start_qpc_ns_ >= 0) {
                const int64_t total_dropout_ms = QpcMs(
                    blend_in_start_qpc_ns_ - dropout_start_qpc_ns_, qpc_freq);
                longest_dropout_ms_ = std::max<uint32_t>(longest_dropout_ms_,
...
double DropoutState::blend_alpha(int64_t qpc_ns, int64_t qpc_freq) const
{
    if (state_ == TrackerState::BLEND_OUT && dropout_start_qpc_ns_ >= 0) {
        ...
        return std::clamp(static_cast<double>(age) /
                          static_cast<double>(std::max<uint32_t>(1, timings_.blend_out_ms)),
                          0.0, 1.0);
    }
    if (state_ == TrackerState::BLEND_IN && blend_in_start_qpc_ns_ >= 0) {
        const int64_t age = QpcMs(qpc_ns - blend_in_start_qpc_ns_, qpc_freq);
        return std::clamp(static_cast<double>(age) /
                          static_cast<double>(std::max<uint32_t>(1, timings_.blend_in_ms)),
                          0.0, 1.0);
    }
    return 0.0;
}
...
uint32_t DropoutState::dropout_age_ms(int64_t qpc_ns, int64_t qpc_freq) const
{
    ...
    return static_cast<uint32_t>(std::max<int64_t>(0,
        QpcMs(qpc_ns - dropout_start_qpc_ns_, qpc_freq)));
}
